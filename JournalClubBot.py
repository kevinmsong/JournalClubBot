...
def main():
    ...
    if uploaded_file is not None:
        ...
        if st.button("Process"):
            st.write(f"Processing {feature}...")

            if feature == "Background Context":
                result = generate_background_context(content, chat_model)
                st.markdown(result)

            elif feature == "Paper Summary":
